...
//!
//! ## Exit codes: the crate's 0 / 2 / 3 / 4
//!
//! The retired oracle's failure code 1 is now the ordinary crate violation 2;
//! bad argv remains `GateError::Usage` (3), and unreadable inputs are errors (4).
...
/// Execute the gate using the product-crate error contract. The report is
/// printed before the typed result, matching the dispatcher contract without
/// terminating the process from inside the implementation.
pub fn run_gate(ctx: &GateCtx) -> Result<(), GateError> {
    ...
    if code == 0 {
        ...
        if failures.is_empty() {
            Err(GateError::violation([format!(
                "verify-injection-count reported a non-zero result without a failure detail"
            )]))
        } else {
            Err(GateError::violation(failures))
        }
    }
}

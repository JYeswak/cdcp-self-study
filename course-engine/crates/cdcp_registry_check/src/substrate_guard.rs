...
use crate::substrate_vcs as vcs;
...
mod date {
    ...
    pub fn parse_ymd(value: &str) -> Result<Ymd, String> {
        let bytes = value.as_bytes();
        if bytes.len() != 10 || bytes[4] != b'-' || bytes[7] != b'-' {
            return Err(format!("{value:?} is not a strict YYYY-MM-DD date"));
        }
        if !bytes
            .iter()
            .enumerate()
        ...
        let year = value[0..4]
            .parse()
            .map_err(|_| format!("{value:?}: bad year"))?;
        let month = value[5..7]
            .parse()
            .map_err(|_| format!("{value:?}: bad month"))?;
        let day = value[8..10]
            .parse()
            .map_err(|_| format!("{value:?}: bad day"))?;
        if !(1..=12).contains(&month) {
            return Err(format!("{value:?}: month {month} out of range"));
        }
        let max_day = days_in_month(year, month);
        if day == 0 || day > max_day {
            return Err(format!("{value:?}: day {day} out of range for month {month}"));
        }
        Ok((year, month, day))
    }

    pub fn before(left: Ymd, right: Ymd) -> bool {
        left < right
    }
}
...
impl GateCtx {
    ...
    fn reject_unknown_flags(&self, known: &[&str]) -> Result<(), GateError> {
...
pub fn evaluate(ctx: &GateCtx) -> Result<(), GateError> {
...
mod tests {
    use super::*;
    use crate::shell_walk as sw; // test-only; production never calls these
...
"#;
    ...
    fn live_check_sh_names_the_first_level_scripts() {
        let root = cdcp_root::walk_engine_root(std::path::Path::new(env!("CARGO_MANIFEST_DIR")))
            .expect("engine root");

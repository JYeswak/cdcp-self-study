...
/// Audit/research scratch is walker-skipped, not [[exclude]]d — those paths
/// are absent on CI and a dead exclusion is ERROR (sm4g.20).
...
// ── product-owned evaluator ────────────────────────────────────────────────

/// The gate result without a dependency on cdcp_gate's dispatcher types.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub fn evaluate_root(root: &Path, args: &[String]) -> GateOutcome {
    if let Some(other) = args.iter().find(|arg| arg.as_str() != "--quiet") {
        return GateOutcome::Usage(format!("unknown argument {other:?}; known: --quiet"));
    }
    ...
    let text = match std::fs::read_to_string(&reg_path) {
        Ok(text) => text,
        Err(e) => {
            return GateOutcome::Error(format!("read {}: {e}", reg_path.display()));
        }
    };
    ...
    if !schema.is_empty() {
        return GateOutcome::Error(format!(
    ...
    let docs = match walk_markdown(&corpus) {
        Ok(docs) => docs,
        Err(e) => return GateOutcome::Error(e),
    };
    ...
    let reserved = match load_reserved_charter_cells(&read) {
    ...
    if !rep.errors.is_empty() {
        return GateOutcome::Error(rep.errors.join(" | "));
    }
    if !rep.violations.is_empty() {
        return GateOutcome::Violation(rep.violations);
    }

    if quiet {
        GateOutcome::Ok(String::new())
    } else {

...
/// The decimal value of a Unicode `Nd` code point. Python's `re` `\d` and the
/// `int()` calls applied to its capture groups accept every Unicode decimal
/// digit; ASCII-only scanning would silently lose milestone rows.
fn py_digit_value(c: char) -> Option<u8> {
    const BLOCKS: &[u32] = &[
        ...
        0x1CCF0, 0x1E140, 0x1E2F0, 0x1E4F0, 0x1E5F1, 0x1E950, 0x1FBF0,
...
fn digits_at(cs: &[char], mut i: usize) -> (usize, String) {
    ...
    while i < cs.len() {
        let Some(d) = py_digit_value(cs[i]) else {
            break;
        };
...
fn scan_root(engine_root: &Path, args: &[String]) -> Result<PathBuf, String> {
        ...
        match args[i].as_str() {
            "--repo-root" => {
                let Some(v) = args.get(i + 1) else {
                    return Err("--repo-root needs a directory".into());
                };
...
/// Product-owned result for the byte-exact roadmap/doc oracle.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DocConsistencyOutcome {
    Output { text: String, code: u8 },
    Usage(String),
}

pub fn evaluate_root(engine_root: &Path, args: &[String]) -> DocConsistencyOutcome {
    let root = match scan_root(engine_root, args) {
        Ok(root) => root,
        Err(message) => return DocConsistencyOutcome::Usage(message),
    };
    ...
    if !errors.is_empty() {
        ...
        return DocConsistencyOutcome::Output { text: out, code: 1 };
    }
...
mod tests {
    ...
    fn crate_exit_code_splits_vacuous_from_violation() {
        assert_eq!(crate_exit_code(&[]), EXIT_OK);
        assert_eq!(

//! Knowledge-path integrity: every `primary_notes` pointer lands on a real file.
//!
//! Extracted from `cdcp_gate/src/gates/verify_knowledge_paths.rs` by
//! `bd-engine-not-gate-ar39.11` (EXTRACT-THEN-DELETE). This is Learn product,
//! not a gate file: a learner reads the notes those pointers name, so a
//! dangling `primary_notes` is a product defect. The `cdcp_gate
//! verify-knowledge-paths` subcommand is a thin dispatcher so `check.sh`
//! does not change. Dual-path `scripts/verify_knowledge_paths.py` was
//! already retired (jhd.33); this extract does not resurrect it.
//!
//! Originally ported as bd-substrate-rust-migration-jhd.5.
//!
//! # CLAIM: FLOOR-RAISE
//!
...
/// `knowledge/`.
pub fn evaluate(root_str: &str) -> Result<Outcome, String> {
    ...
    let text = std::fs::read_to_string(&domains_path).map_err(|e| {
        format!("{domains_path}: unreadable or not UTF-8: {e} (the oracle raises here)")
    })?;
    let data: toml::Table = text
        .parse()
        .map_err(|e| format!("{domains_path}: TOML parse error: {e} (the oracle raises here)"))?;
    ...
    for name in glob_toml_sorted(&knowledge_dir) {
        ...
        let text = std::fs::read_to_string(&full).map_err(|e| {
            format!("{full}: unreadable, a directory, or not UTF-8: {e} (the oracle raises here)")
        })?;
...
mod tests {
    ...
    fn malformed_domains_toml_is_an_error_not_a_pass() {
        ...
        assert!(err.contains("the oracle raises here"), "{err}");
    }
    ...
    fn header_states_a_floor_raise_and_overclaims_nothing() {
        let src = include_str!("knowledge_paths.rs");

...
/// `http` (types) is included because the AC forbids an http dependency
/// entering the lockfile, not only a named client.
const BANNED: &[&str] = &[
    "reqwest",
    "reqwest-middleware",
    "ureq",
...
#[test]
fn cdcp_data_manifest_has_no_http_dep() {
    ...
    for needle in ["reqwest", "ureq", "hyper", "attohttpc", "isahc"] {
        assert!(
            ...
            "cdcp_data Cargo.toml mentions {needle}"
        );
    }
}

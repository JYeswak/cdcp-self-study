//! Domain coverage floors: every module the course DECLARES carries at
//! least its floor of APPROVED bank items.
//!
//! Extracted from `cdcp_gate/src/gates/verify_coverage.rs` by
//! `bd-engine-not-gate-ar39.14` (EXTRACT-THEN-DELETE). This is bank product,
...
/// the coverage gate does not narrow the Python oracle's accepted input.
...
/// `Err` is a usage message (the dispatcher maps it to exit 3).

//! Topic↔bank referential integrity over the approved pool.
//!
//! Extracted from `cdcp_gate/src/gates/verify_orphans.rs` by
//! `bd-engine-not-gate-ar39.9` (EXTRACT-THEN-DELETE). This is bank product,
...
fn py_is_printable(c: char) -> bool {
    ...
    !matches!(c as u32,
        0x00ad | 0x0378..=0x0379 | 0x0380..=0x0383 | 0x038b | 0x038d
        | 0x0890..=0x0891
...
/// unambiguous prefixes argparse allows. `Err` is a usage message (gate maps
/// it to exit 3).
...
mod tests {
    ...
    fn an_unknown_flag_is_usage_never_a_silent_pass() {
        ...
        assert!(
            ...
            "a typo must not read as a default-path run: {err}"
        );
    }
    ...
    fn a_flag_without_its_value_is_usage() {
        ...
        let err = parse_args(&args).unwrap_err();
        assert!(
            err.contains("expected one argument"),
            "a flag without its value must be usage: {err}"
        );
    }
    ...
    fn header_states_a_floor_raise_and_overclaims_nothing() {
        let src = include_str!("orphans.rs");

//! Bank pool floors: schema, topic ids, domain minimums, letter diversity.
//!
//! Extracted from `cdcp_gate/src/gates/verify_bank.rs` by
...
//!   sides differ on every RED case. The thin `cdcp_gate` dispatcher therefore
//!   does not route a RED `Outcome` through `GateError`. (An invocation error —
//!   this check takes no arguments — still returns `GateError::Usage` from the
//!   dispatcher, since the oracle has no opinion there.)
...
fn unicode_decimal_digit(c: char) -> Option<u8> {
    const BLOCKS: &[u32] = &[
        ...
        0x16AC0, 0x16B50, 0x16D70, 0x1CCF0, 0x1E140, 0x1E2F0, 0x1E4F0, 0x1E5F1, 0x1E950, 0x1FBF0,

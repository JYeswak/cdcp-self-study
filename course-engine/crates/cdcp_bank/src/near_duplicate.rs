//! Textual near-duplicate detection over the assembly pool
//! (bd-near-duplicate-item-gate-i5v).
//!
//! Extracted from `cdcp_gate/src/gates/near_duplicate_items.rs` by
//! `bd-engine-not-gate-ar39.7` (EXTRACT-THEN-DELETE). This is bank product,
//! not a gate file: C3 lives next to [`crate::paraphrase`] because both
//! measure the approved pool a learner can still draw twice. The gate
//! subcommand is a thin dispatcher so `check.sh` does not change.
//!
//! # CLAIM: FLOOR-RAISE
//!
//! **No two assembly-eligible bank items may be textually close enough to
//! read as the same question twice.** Concretely this goes RED when any pair
//! of `status = "approved"` items clears one of the two rules in [`Rule`],
//! and it names both items, both files, and all three measured similarities
//! so the reader can adjudicate the pair without opening anything.
//!
...
//!
//! The detector is additionally required to be shown TRIPPING, not merely
//! passing: `CDCP_NEAR_DUPLICATE_SELFTEST=1` injects a cosmetically-reworded
//! clone of a real approved item into the in-memory pool and asserts the
//! clone is flagged against its source. That run is `Ok` only when the RED
//! path was reached, and `Error` when the detector failed to catch its own
//! planted known-bad.
...
/// Outcome of one evaluation. The gate dispatcher maps `Ok` → exit 0,
/// `Violation` → exit 2, `Error` → exit 4.
...
/// Evaluate the approved pool under `root`. Honours [`SELFTEST_ENV`].
pub fn evaluate(root: &Path) -> Eval {
    evaluate_with(root, std::env::var(SELFTEST_ENV).unwrap_or_default() == "1")
}

/// Same as [`evaluate`], with the injected-known-bad switch passed in so
/// tests do not race on a process-global env var.
pub fn evaluate_with(root: &Path, selftest_only: bool) -> Eval {
    let dir = root.join(BANK_REL);
    let (items, files) = match load_items(&dir) {
        Ok(v) => v,
    ...
    if selftest_only {
        return match selftest(&approved) {
            Ok(msg) => Eval::Ok(format!("{msg}\n")),
            Err(e) => Eval::Error(e),
        };
    }

    match find_near_duplicates(&approved) {
        Err(e) => Eval::Error(e),
        Ok((findings, comparisons)) => {
            if comparisons == 0 {
                return Eval::Error(
            ...
            if !findings.is_empty() {
                return Eval::Violation(findings.iter().map(|f| f.line()).collect());
            }
            Eval::Ok(format!(

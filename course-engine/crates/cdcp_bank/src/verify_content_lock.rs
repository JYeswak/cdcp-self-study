//! Content lock: pinned `bank_hash` and knowledge/module digests still match
//! the tree, in both directions.
//!
//! Extracted from `cdcp_gate/src/gates/verify_content_lock.rs` by
//! `bd-engine-not-gate-ar39.13` (EXTRACT-THEN-DELETE). This is bank product,
//! not a gate file: `content.lock` pins the bank content address and the
//! knowledge/module trees a learner is taught from, so a drifted pin or an
//! unpinned file is a product defect. The `cdcp_gate verify-content-lock`
//! subcommand is a thin dispatcher so `check.sh` does not change. Dual-path
//! oracle `scripts/verify_content_lock.py` stays retired (gna0); this extract
//! does not resurrect it.
//!
//! Originally ported as bd-substrate-rust-migration-jhd.8.
//!
//! # CLAIM: FLOOR-RAISE
//!
...
/// Undecidable states (ill-formed TOML, non-UTF-8, unreadable pin). Same
/// numeric as `cdcp_gate::exit::ERROR` (4). The oracle raises a traceback
/// (exit 1) on these; this port reports 4 so a crash cannot read as a match.
const ERROR: i32 = 4;
...
mod tests {
    ...
    /// Green path of `evaluate` on a constructed tree. The live-repo pin lives
    /// on the gate dispatcher (`the_live_tree_is_green_and_the_receipt_names_the_counts`)
    /// so this crate's `--lib` tests do not fail closed on another agent's
    /// in-flight bank / lock edits.
    #[test]
    fn a_fully_pinned_fixture_is_green_and_the_receipt_names_the_counts() {
        let (_td, engine, ..) = walkable_root();
        let ka = sha256_hex_bytes(b"x = 1\n");
        ...
        assert!(
            out.stdout.contains(
                "verify_content_lock: covered roots (every file found under these is pinned \
                 and matched): knowledge/*.toml=1 web/content/modules/*.md=1 ../modules/*.md=1\n"
            ),
            "{}",
            out.stdout
        );

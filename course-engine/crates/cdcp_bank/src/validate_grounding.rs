//! Bank-item grounding: no uncited clause/setpoint/dump language, and
//! (under `--strict-overlap`) lexical overlap with the teaching corpus.
//!
//! Extracted from `cdcp_gate/src/gates/validate_grounding.rs` by
//! `bd-engine-not-gate-ar39.13` (EXTRACT-THEN-DELETE). This is bank product,
//! not a gate file: a learner is assessed on these stems, so an item that
//! cites a clause the tree cannot check, or that shares no vocabulary with
//! the corpus, is a product defect. The `cdcp_gate validate-grounding`
//! subcommand is a thin dispatcher so `check.sh` does not change. Dual-path
//! oracle `scripts/validate_grounding.py` stays.
//!
//! Originally ported as bd-substrate-rust-migration-jhd.9.
//!
//! # CLAIM: FLOOR-RAISE
//!
...
//! check, exactly as in `cdcp_bank::orphans`. That is a knowing, single-file
...
fn py_digit_value(c: char) -> Option<u8> {
    const BLOCKS: &[u32] = &[
        ...
        0x111d0, 0x112f0, 0x11450, 0x114d0, 0x11650, 0x116c0, 0x116d0, 0x11730, 0x118e0, 0x11950,
        0x11bf0, 0x11c50, 0x11d50, 0x11da0, 0x11f50, 0x16130, 0x16a60, 0x16ac0, 0x16b50, 0x16d70,
        0x1ccf0, 0x1e140, 0x1e2f0, 0x1e4f0, 0x1e5f1, 0x1e950, 0x1fbf0, 0x1d7ce, 0x1d7d8, 0x1d7e2,
        0x1d7ec, 0x1d7f6,
    ];
    let cp = c as u32;
    BLOCKS
...
fn ascii_decimal_digits(s: &str) -> String {
    s.chars()
        .map(|c| py_digit_value(c).map(|d| char::from(b'0' + d)).unwrap_or(c))
        .collect()
}
...
fn py_is_printable(c: char) -> bool {
    ...
    !matches!(c as u32, 0x00ad | 0x0378..=0x0379 | 0x0380..=0x0383 | 0x038b | 0x038d | 0x0600..=0x0605 | 0x061c | 0x06dd | 0x070f | 0x08e2

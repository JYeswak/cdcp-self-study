...
/// U+038B and U+038D are unassigned code points, so Python's `repr(str)` emits
/// `\u` escapes. The known-bad mutant leaves them printable in Rust, making
...
fn path_and_option_shapes_are_byte_identical_and_still_reach_the_finding() {
    ...
    let owned_scratch = cdcp_registry_check::scratch::ScratchDir::new(&root, "diff-orphans")

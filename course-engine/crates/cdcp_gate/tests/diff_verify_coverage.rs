...
/// Python's `str.isprintable()` rejects the unassigned U+038B and U+038D,
/// so `repr()` escapes them in a malformed bank-module diagnostic. The
/// known-bad mutant leaves those code points printable and emits raw scalars;
...
fn no_bank_loader_iterates_items_without_a_zero_yield_guard() {
    ...
    // `verify_orphans.py` was intentionally retired in the current Rust
    // migration slice; the remaining Python loaders are bank, coverage, and
    // objectives. Keep a nonzero explicit floor so an empty scan cannot pass.
    assert!(
        branches.len() >= 3,

...
**check.sh:** rebuilds `cdcp_wasm` `--release --locked` and asserts sha256 equality with `web/assets/wasm/cdcp_wasm.wasm`. Dual-path loads that shipped blob. If `wasm32-unknown-unknown` is absent the rebuild is **SKIPPED** and the run is **not** advertised as full-green (`GAPS` + skip receipt). `selftest_wasm_freshness.sh` plants two known-bads (`INJECTIONS=2 SUITE=wasm-freshness` [[fact:fact-injections-enforced=yes]]).

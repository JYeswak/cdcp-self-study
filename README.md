...
**A free, offline, self-hosted course that teaches the data-centre facilities domain, plus a Rust engine that grades you the same way twice.** Fifteen modules of original writing (~82,000 words) cover the fourteen publicly advertised EPI® CDCP® facility domains and one ops-adjacent supplement. The engine ships an 869-item question bank (844 approved: a **pool size**, not a count of distinct propositions) and a browser course whose grader is a pure-Rust core compiled to WASM, pinned so the native and browser paths produce **byte-identical** result digests. No account, no telemetry, no network at runtime, no LLM in the grading path.
...
| **Bank** | 869 original item files / 844 approved · 25 retired (pool size, not distinct propositions) [[fact:fact-bank-item-count-804=yes]] [[fact:fact-bank-approved-count-779=yes]] · 15 modules · 106 topics |
...
2. **No exam dumps, ever.** All 869 item files are original, written against public syllabus domains and industry-standard references. `source_class=original` is verified for every item on every run. 869/844 is a file-set / approved-pool size, not a count of distinct propositions.
...
└── course-engine/
    ...
    ├── bank/items/           869 original item files, one TOML each (file count, not distinct propositions)
...
bank/items/*.toml ──► cdcp_bank ──► bank_hash (SHA-256 over the sorted bank)
                                        │
              seed ──► cdcp_assemble ────┤ stratified 40 of 844 approved (869 files)
                                        ▼
...
| — | **L3 external oracle.** Site quantities now check against NREL / USGS / EPA (`cdcp_data/tests/oracle.rs`, 2026-08-15); the 869 bank answer keys remain unguarded | **open, and the most valuable thing to fix** |
...
- **The question bank is unaudited by a third party.** 869 original item files
  (844 approved: a pool size, not a distinct-proposition count), self-reviewed.
...
violation and useless. All 869 item files are original, written against public
syllabus domains.

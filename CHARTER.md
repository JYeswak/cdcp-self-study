...
| **Shipped means (2026-08-12)** | (1) Open the **local-server hub** (`cargo run -p cdcp_cli -- serve` → http://127.0.0.1:8766/). **`file://` is not a supported origin** and fails closed as **CDCP_FILE_ORIGIN** (browsers block ES modules / `fetch` / WASM from a double-clicked HTML file). Then: Learn 15 modules (14 EPI domains + ops-adjacent) → module quiz / Learn-15 / Drill / Mock-40 → score + explanations + weak modules; (2) `./scripts/check.sh` green (W0–L7 + V11); (3) fixed seed exam → **byte-identical** grade digest vs golden (native==WASM); (4) Loop#3 hybrid protocol + ≥1 T1 run log; (5) free/public corpus store policy |
...
| **L5 Adversarial floor** | **PARTIAL · property only** | Corrected 2026-08-14 by B1: the proptest floor IS wired and tested. The fuzz half is present-but-unrun — `fuzz/` is outside the workspace, so no `check.sh` step, no CI job and no `cargo test --workspace` ever builds or runs those targets. bd-p228. |
| **L6 Formal** | NO (v1) | — |
...
layers: L1 L3a L3b L4 L7 wired; L2 smoke-only and L5 property-only (B1, 2026-08-14); L3 factual NO; L6 no; Learn-surface gates M8 wired
